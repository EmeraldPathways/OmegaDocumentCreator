...
# ---------------------------------------------------------------------------
# DB column → frontend SeededClientProfile field name mapping
# ---------------------------------------------------------------------------
...
_PROTECTION_MAP: list[tuple[str, str]] = [
    ...
    ("current_annual_cover", "recommendedCover"),
...
class WorkflowRepository:
    ...
    def get(self, client_id: UUID) -> dict[str, object]:
        """Return a flat dict of workflow fields keyed by frontend SeededClientProfile names."""
        result: dict[str, object] = {}
        ...
        if tob is not None:
            ...
            result["termsStatus"] = tob.status if hasattr(tob, "status") else "draft"
    ...
    def save(self, client_id: UUID, data: dict[str, object]) -> None:
        """Upsert workflow data from a flat dict of frontend field names."""
        ff = self._db.query(FactFind).filter(FactFind.client_id == client_id).first()
        if ff is None:
        ...
        samp = data.get("employmentStatus", "")
        if samp:
            emp = self._db.query(EmploymentDetail).filter(EmploymentDetail.client_id == client_id).first()
            if emp is None:
        ...
        samp = data.get("incomeProtectionDeferredProvider", "") or data.get("deferredPeriod", "")
        if samp:
            prot = self._db.query(ProtectionDetail).filter(ProtectionDetail.client_id == client_id).first()
            if prot is None:
        ...
        samp = data.get("mortgageProtection", "") or data.get("personalInsurance", "")
        if samp:
            life = self._db.query(LifeSeriousIllnessDetail).filter(LifeSeriousIllnessDetail.client_id == client_id).first()
            if life is None:
