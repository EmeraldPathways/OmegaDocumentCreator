...
FROM (
    SELECT
        CAST(SUBSTRING(client_reference FROM '^(?:CLI|Omega)-(\d{4})-\d+$') AS INTEGER) AS year_value,
        MAX(CAST(SUBSTRING(client_reference FROM '^(?:CLI|Omega)-\d{4}-(\d+)$') AS INTEGER)) AS max_sequence
    FROM clients
    WHERE client_reference ~ '^(?:CLI|Omega)-\d{4}-\d+$'
    GROUP BY 1
) AS seeded
